...
## [v0.11.8] - 2026-06-23 17:40

### 修正：「上次 refresh」時間戳時區錯誤（雲端寫 UTC，顯示像早上 → 誤導成沒更新）

**問題（user 回報，合理動怒）**：網頁「上次 refresh 2026-06-23 09:14 ✓」看起來停在早上、
像是強制 refresh 後就沒再更新、資訊在騙人。

**根因**：`refresh.py` 寫 `refresh_log.ts` 用 `datetime.now()`（**無時區 = 機器當地時間**）。
本機（台北）跑寫台北時間、**雲端 runner（UTC）跑寫 UTC**，前端又原樣顯示、未標時區。
自動 refresh 全在雲端 → 一律寫成比台北早 8 小時的 UTC → 每次更新都像「早上 9 點多」。
實際上資料有在更新（如 09:31 UTC = 台北 17:31，就是當下剛跑的）。DB 內時間戳因此 UTC／
台北混雜。

**修正**：
- `refresh.py`：`refresh_log.ts` 改用**固定 UTC+8**（`timezone(timedelta(hours=8))`，台灣無
  日光節約）寫入 → 不論本機或雲端跑都是台北時間，且不需 `tzdata` 套件。
- 前端 `app.js`（`app/static` + `docs` 兩份）：去掉時區尾碼、明確標「(台北)」。
- 既有舊紀錄（UTC／台北混雜）不回填：只有「最新一筆」會被顯示（dates.json `last_refresh`），
  下次 refresh 後即自動顯示為正確台北時間，歷史列不對外顯示故無影響。

## [v0.11.7] - 2026-06-23 17:11

### 修正：每日自動更新沒在跑 —— GitHub 內建排程實測不觸發，改用外部排程器

**問題（user 回報）**：打開網站停在「For 2026/6/23 開盤前看」，日曆切到 6/24 會被打回
6/23、切不過去。

**排查（全用線上資料 + CI 紀錄驗證，非推測）**：
- 線上最新「有資料的交易日」只到 6/22 → 最新視圖就是 For 6/23；選 6/24 經
  `resolveDataDate`（嚴格小於）落回 6/22 的檔，該檔內建 `view_date=6/23`，前端
  `app.js:212` 又把日曆設回 6/23 → 「跳不過去」。根因是**資料沒更新**，不是日曆 bug。
- 為何沒更新：`update.yml` 的 cron 是今天 11:19 才第一次加入，且 **`event=schedule`
  的執行從頭到尾 0 筆**。push / 手動 / pages-build 全正常 → runner、額度、infra 沒問題，
  **單單 GitHub 內建排程觸發器不動**。兩次測試班（11 分 / 34 分提前量，後者避開整點）
  排程時間到都沒跑 → 判定 GitHub 內建 cron 對此 repo 不可靠，不能當依靠。

**修正**：
- 自動更新「主力」改走**外部排程器**（cron-job.org → 打 `workflow_dispatch` API），
  完整設定與排查紀錄寫進 `docs/SCHEDULING.md`，README 對應段落更正。
- `update.yml` 的 3 班 cron：分鐘 `0`→`17`（避開整點高負載時段）並降級為**備援**，
  移除排查用的暫時測試班。
- 立即手動觸發一次 `update-data` 解套今天：線上已更新到 data_date 6/23、view 6/24，
  6/24 可正常選取（6/23 夜盤欄待今晚 / 明早班次漸進補上）。

**待辦（未做，待 user）**：在 cron-job.org 完成 PAT + job 設定（步驟見 docs/SCHEDULING.md），
之後每日自動更新才真正生效。
...
靜態網站沒有後端可抓，原本的「重新整理」按鈕只是 reload，意義不大。改為：

- 按鈕改顯示「🕒 下次自動更新 今天/明天 HH:00」，依當下台北時間自動算出下一個
  cron 時段（07:00 / 15:00 / 21:00），並設為不可點。
- `app.js`（主表）與 `comprehensive.html`（綜合整理）都改。
- header 仍保留「上次 refresh」→ user 同時看得到上次與下次更新。

> 註：本版未處理「跨假日夜盤日期歸屬」的既有 bug（連假時夜盤被 TAIFEX 標在
> 假日當天、未自動重貼回假期前最後交易日，導致 dashboard 該日夜盤欄空白）。
> 該問題待另案修正 + 對 Excel 驗證。

## [v0.11.4] - 2026-06-23 11:51

### 修正：README 澄清手動觸發是維護者專用（避免誤導成「人人可按」）

- `workflow_dispatch` 即使 public repo 也只有「有 repo 寫入權限者」能觸發，
  一般訪客看得到 Actions 紀錄但無法執行。原 README 寫法易讓人誤以為任何人都能按。
- 改寫該行：標明〔維護者專用〕、只有你能觸發、訪客按不了。
- 未改動實際行為（手動觸發本就安全，且與 cron 做的事相同）。

## [v0.11.3] - 2026-06-23 11:48

### 修正：README 開頭改成 user 真正要的資訊

原本 README 一開頭就是「目的 / 架構 / Schema」等開發者內容，連線上網址都埋在
第 68 行的小項目裡，user 找不到。改為：

- 開頭新增「🔗 直接看這裡」區塊：醒目放上 Pages 網址 + 三個頁面（主表 / 綜合整理 /
  走勢圖）連結與用途、「每天自動更新 3 次」「怎麼手動更新」「主表可選日期」。
- 技術文件（架構 / Schema / endpoint 等）退到分隔線之後，標明「給開發者 / 維護者看」。
